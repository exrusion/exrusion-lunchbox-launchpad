...
app.get("/v1/media/:id",async(req,reply)=>{const {rows}=await query("SELECT mime_type,bytes FROM media_uploads WHERE id=$1",[req.params.id]);if(!rows[0])return reply.code(404).send({error:"Image not found"});reply.header("content-type",rows[0].mime_type).header("cache-control","public,max-age=31536000,immutable").header("cross-origin-resource-policy","cross-origin").send(rows[0].bytes);});
