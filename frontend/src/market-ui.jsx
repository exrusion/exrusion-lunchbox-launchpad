...
export function PadBuilderForm({form,setForm,onSubmit,busy}){
 ...
 return <form onSubmit={onSubmit}>
  <div className="two"><label>Launchpad name<input required minLength="2" maxLength="48" value={form.name} onChange={e=>{const name=e.target.value;setForm({...form,name,slug:name.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"").slice(0,32),agentName:form.agentName&&form.agentName!=="My Lunchmate"?form.agentName:`${name||"My"} Lunchmate`})}}/></label><label>Storefront subdomain<input required pattern="[a-z][a-z0-9-]{2,31}" value={form.slug} onChange={e=>setForm({...form,slug:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,"")})}/><small>Publishes at <b>{form.slug||"your-pad"}.lunchpad.family</b></small></label></div>
