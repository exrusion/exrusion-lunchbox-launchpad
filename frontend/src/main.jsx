...
function App(){
 ...
 async function createPad(e){
  e.preventDefault();
  try{
   setBusy("Checking the kitchen before we serve…");setNote("");
   ...
   const jwt=await auth();setBusy("Plating your lunchpad art…");const logoUrl=await uploadMedia(form.image,jwt);
   setBusy("Packing every compartment…");
   ...
   const padId=keccak256(toHex(pad.id));setBusy("Sealing the lid onchain…");
   ...
   if(form.marketType!=="agents"||agentReady)setNote(form.marketType==="agents"?"Lunchpad and lunchmate packed, sealed and live.":"Lunchpad packed, sealed and ready to serve.");
 ...
 <section className="hero"><div className="eyebrow"><Sparkles size={15}/> INSPIRED BY <a href="https://x.com/MEADGod" target="_blank" rel="noreferrer" title="Ozzy on X">OZZY · PONS DEV</a></div><h1>Pack your own<br/><em>launchpad.</em></h1><p>Create a branded token launchpad with isolated ownership and automatic fee routing. You bring the big idea. We pack the rails.</p><div className="actions"><button className="primary" onClick={()=>openBuilder()}>Pack my launchpad <ChevronRight/></button><a className="ghost" href="#directory">See what’s on the menu</a></div><div className="fee"><span><b>70%</b> creator</span><span><b>20%</b> lunchpad owner</span><span><b>10%</b> protocol</span></div><LunchArt/></section>
 <section id="subdomains" className="subdomainBand"><div><span className="kicker">ONE BOX. ONE ADDRESS.</span><h2>your-pad.lunchpad.family</h2><p>Every lunchpad gets its own seat at the table: one clean subdomain for its storefront, launches and AI lunchmate.</p></div><button className="primary" onClick={()=>openBuilder()}>Reserve my table <ChevronRight/></button></section>
 <TechSections onBuild={openBuilder}/><section id="directory" className="directory"><div className="sectionHead"><div><span className="kicker">TODAY'S MENU</span><h2>Fresh out of the box</h2></div><button className="round" aria-label="Create a lunchpad" onClick={()=>openBuilder()}><Plus/></button></div>{note&&<div className="notice" role="status">{note}</div>}<div className="grid">{pads.length?pads.map(p=><article className="card" key={p.id} style={{"--accent":p.accent}}><div className="cardTop"><div className="padLogo"><PadLogo src={p.logoUrl} alt={`${p.name} logo`}/></div><span className={`status ${p.status}`}>{p.status}</span></div><h3>{p.name}</h3><p>{p.description}</p><a className="storefrontAddress" href={storefrontUrl(p.slug)}>{p.slug}.lunchpad.family <ExternalLink size={13}/></a><div className="meta"><span>{p.niche}</span><span>{p.creatorTaxBps/100}% tax</span><span>{p.launches||0} launches</span></div><div className="cardLinks"><a href={storefrontUrl(p.slug)}>Open this lunchbox <ExternalLink size={15}/></a><a href={`/admin/${p.slug}`}>Run the kitchen</a></div></article>):<div className="empty"><LunchboxIcon/><h3>No lunchpads on the table yet.</h3><p>Be the first to bring lunch.</p><button className="primary" onClick={()=>openBuilder()}>Pack the first lunchpad</button></div>}</div></section>
 <section id="how" className="how"><span className="kicker">HOW WE PACK IT</span><div className="steps"><div><b>01</b><h3>Choose the filling</h3><p>Name it, theme it and choose a 2–4% creator tax.</p></div><div><b>02</b><h3>Seal the lid</h3><p>Your wallet registers isolated lunchpad ownership onchain.</p></div><div><b>03</b><h3>Serve it hot</h3><p>Pons V2 launches route every fee into its verified compartment.</p></div></div></section>
 {walletOpen&&<Modal label="Choose a wallet" close={()=>{setWalletOpen(false);setWalletError("");setPendingPreset(null)}}><h2>Pick who carries the lunch money</h2><p>Choose deliberately. Lunchbox opens only for the wallet you select.</p>{walletError&&<div className="notice" role="alert">{walletError}</div>}<div className="walletList">{walletOptions.length?walletOptions.map(x=><button key={x.key} onClick={()=>connect(x)}>{x.label}<ChevronRight/></button>):<div className="notice">No supported wallet detected. Install or unlock MetaMask, Trust Wallet, or Phantom, then reopen this menu.</div>}</div></Modal>}
 {open&&<Modal label="New lunchpad deployment" close={()=>setOpen(false)}><span className="kicker">NEW LUNCHPAD DEPLOYMENT</span><h2>Pack a business, not just lunch</h2><p>Choose every compartment: storefront, operating lane and public identity. Your address becomes a live <b>slug.lunchpad.family</b> subdomain.</p><PadBuilderForm form={form} setForm={setForm} onSubmit={createPad} busy={busy}/></Modal>}
 </main>
}
function PadPage({slug}){
 ...
 async function launch(e){
  ...
  try{
   ...
   setNote("Lunch served hot. Your token is live.");
 ...
 if(!data)return <main><header><Brand/></header><section className="directory"><div className="notice">{note||"Unpacking today's menu…"}</div></section></main>;
 const providerLabel=isCme?"CME V6":marketType==="agents"?"AI17Z + Pons V2":"Pons V2";
 return <main><header><Brand/><a className="wallet" href={mainSiteUrl}>Back to the cafeteria</a></header>
  ...
  </section><AgentChat slug={slug} pad={data.pad}/><section className="directory"><span className="kicker">FRESHLY SERVED</span><div className="grid">{data.launches.length?data.launches.map(l=><article className="card" key={l.id} style={{"--accent":data.pad.accent}}><div className="cardTop"><div className="padLogo"><PadLogo src={l.token_logo_url} alt={`${l.token_name} logo`}/></div><span className="status active">live</span></div><h3>{l.token_name} <small>${l.token_symbol}</small></h3><p>{l.market_target_label||providerLabel} · {short(l.token_address)}</p><a href={`https://robinhoodchain.blockscout.com/tx/${l.transaction_hash}`} target="_blank" rel="noreferrer">View transaction <ExternalLink size={15}/></a></article>):<div className="empty"><Box/><h3>This lunchbox is still unopened.</h3><p>Be the first launch on the menu.</p></div>}</div></section></main>;
}
...
function LunchArt(){return <img className="brandHeroArt" src="/lunchpad-hero.webp" alt="" aria-hidden="true"/>}
